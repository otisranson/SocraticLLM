...
- [ ] Guardrail (the hard constraint) implemented — first pass done (`socraticllm/engine/guardrail.py`, structural/heuristic only); dialogue engine still to come
- [ ] Dialogue engine implemented (wires `LLMClient` + guardrail + concept graph into a turn loop)
...
**Next task:** Dialogue engine (`socraticllm/engine/dialogue.py`) — wire `LLMClient` + the guardrail + the concept graph into a turn-taking loop, informed by the Pedagogy section above (don't resolve prematurely, notice easy acceptance, name understanding when it arrives). On a guardrail rejection the loop needs a retry strategy (e.g. re-prompt with the rejection reason) — not yet designed.

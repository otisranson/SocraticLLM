...
**Last session (2026-07-13):** Built out the whole engine stack from scratch, in order, each with passing tests before the next started. Prior sessions had reconciled the Socratic-tutor vision (`VISION.md`, this file, `TODO.md` rewritten to match; see git history around `90d1a72`/`9e5cc56`/`339915a` for that and the legacy-transformer extraction/concept-graph-schema detail) — this session picked up from there:
...
Also recorded two forward-looking product ideas as Open Design Questions during a conversation about API key handling — not scoped for v1, just flagged so they aren't lost: a possible multi-tenant admin UI (teacher registration, IT-admin-configured key), and whether students need accounts at all versus an unauthenticated school-DNS-pointed deployment.

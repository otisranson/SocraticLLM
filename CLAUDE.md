...
**Then:** Hand-authored the first-version curriculum. Asked the user which subject; chose algebra (linear equations) over "how LLMs work" — it matches `VISION.md`'s own "First Version" example, and a small algebra concept graph already existed as a test fixture in `tests/test_concept_graph.py` (`variables` → `linear-equations` → `systems-of-equations`), so this reuses a shape already proven to validate cleanly rather than inventing a new one. `socraticllm/curriculum/algebra_1.py`'s `build_algebra_1()` returns a fresh `ConceptGraph` (curriculum id `"algebra-1"`) with 8 concepts forming the recognition ladder `VISION.md` describes — "session ends when student has identified problem type and approach": `variables` / `equality` / `inverse-operations` / `like-terms` (foundations, no prerequisites among themselves) → `one-step-equations` → `two-step-equations` → `multi-step-equations` (also requires `like-terms`) → `systems-of-equations`. Each concept has a real description (not a placeholder) written for what a student needs to recognize, not just a label. `graph.validate()` runs at the end of `build_algebra_1()` itself, so a broken graph fails at construction, not later. Re-exported as `build_algebra_1` from `socraticllm/curriculum/__init__.py`. `tests/test_algebra_1.py` (5 cases: valid graph, expected concept ids present, topological order respects every prerequisite edge, the recognition chain matches the intended ladder exactly, two calls return independent graphs — mutating one's `Concept.description` doesn't affect the other) passing. Deliberately did not wire this into `DialogueEngine` yet (that's `TODO.md` step 7, the REPL) — this step is just the content. All 47 tests pass.

**Next task:** Student-side REPL (`socraticllm/interface/repl.py`, `TODO.md` step 7) — wire `build_algebra_1()` + student model + `DialogueEngine` + guardrail into one working loop: student states a problem, dialogue proceeds via questions only, session ends on demonstrated recognition, student state updates. This is also where the `Problem`/session-model gap flagged in Open Design Questions will likely need to get resolved — there's still no representation of "which problem is the student working on right now," which the REPL will need in order to pick a `curriculum_context` string for `DialogueEngine` and to know which `algebra_1` concept id to update in `StudentState` when the student demonstrates recognition.
